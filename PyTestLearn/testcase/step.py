...
"""
@pytest.mark.parametrize("test_input,expect_result",[("input1","expected1"),("input2","expected2")])
def test_example(test_input,expect_result):

    actual_result = perfmon_operation(test_input)
    step1 = "Execute A"
    step2 = "Execute B"
    expect_result = 10
...
if __name__ == "__main__":
    ...
    # pytest.main(['-s'])
    # #指定模块 中标记为smoke的测试用例
    pytest.main(['-sv', 'step.py', '--html=../report/step.html', '--self-contained-html'])
